...
def categories_results_url(budget_id: int) -> str:
    """Create and return a categories results URL."""
    return reverse("charts:categories-in-periods-chart", args=[budget_id])
...
class TestCategoriesInPeriodsChartAPIView:
    ...
    def test_auth_required(self, api_client: APIClient, budget_factory: FactoryMetaClass):
        ...
        response = api_client.get(categories_results_url(budget.id))
    ...
    def test_auth_with_jwt(
        ...
        url = categories_results_url(budget.id)
    ...
    def test_user_not_budget_member(
        ...
        """
        GIVEN: Budget instance in database.
        WHEN: CategoriesInPeriodsChartAPIView called with GET by User not belonging to given Budget.
    ...
    def test_get_categories_results_empty_budget(
        self,
        api_client: APIClient,
        base_user: AbstractUser,
        budget_factory: FactoryMetaClass,
        ...
        """
        GIVEN: Budget with no periods and no categories in database.
        WHEN: CategoriesInPeriodsChartAPIView called by Budget member.
    ...
    def test_get_categories_results_no_periods(
        ...
        """
        GIVEN: Budget with categories but no periods in database.
        WHEN: CategoriesInPeriodsChartAPIView called by Budget member.
        THEN: HTTP 200 - Response with empty xAxis and series arrays returned.
        """
        budget = budget_factory(members=[base_user])
    ...
    def test_get_categories_results_no_categories(
        ...
        """
        GIVEN: Budget with periods but no categories in database.
        WHEN: CategoriesInPeriodsChartAPIView called by Budget member.
        THEN: HTTP 200 - Response with empty xAxis and series arrays returned.
        """
        budget = budget_factory(members=[base_user])
    ...
    def test_get_categories_results_basic_scenario(
        ...
        """
        GIVEN: Budget with periods and categories but no transfers.
        WHEN: CategoriesInPeriodsChartAPIView called by Budget member.
    ...
    def test_get_categories_results_with_transfers(
        ...
        """
        GIVEN: Budget with periods, categories, and transfers.
        WHEN: CategoriesInPeriodsChartAPIView called by Budget member.
        ...
        groceries_category = transfer_category_factory(
            budget=budget, deposit=deposit1, name="Groceries", category_type=CategoryType.EXPENSE
        )
        interest_category = transfer_category_factory(
            budget=budget, deposit=deposit2, name="Interest", category_type=CategoryType.INCOME
        )
        ...
        response = api_client.get(
            categories_results_url(budget.id), {"period_from": period2.id, "period_to": period3.id}
        )
        ...
        response = api_client.get(categories_results_url(budget.id), {"category_type": CategoryType.INCOME})

        assert response.status_code == status.HTTP_200_OK
        assert len(response.data["series"]) == 2  # Only income categories

        # Test with specific category filtering
        response = api_client.get(categories_results_url(budget.id), {"category": salary_category.id})
        ...
        response = api_client.get(categories_results_url(budget.id), {"deposit": deposit1.id})
    ...
    def test_large_dataset_performance_simulation(
        ...
        """
        GIVEN: Budget with many periods, categories, and transfers to simulate real-world load.
        WHEN: CategoriesInPeriodsChartAPIView called by Budget member.
    ...
    def test_edge_cases_and_error_handling(
        ...
        """
        GIVEN: Budget with various edge cases (zero values, missing data).
        WHEN: CategoriesInPeriodsChartAPIView called by Budget member.
        ...
        transfer_category_factory(
            budget=budget, deposit=deposit, name="Zero Transfers", category_type=CategoryType.EXPENSE
        )
        normal_category = transfer_category_factory(
            budget=budget, deposit=deposit, name="Normal", category_type=CategoryType.INCOME
        )
        partial_category = transfer_category_factory(
            budget=budget, deposit=deposit, name="Partial", category_type=CategoryType.EXPENSE
        )
    ...
    def test_different_budgets_isolation(
        self,
        base_user: AbstractUser,
        budget_factory: FactoryMetaClass,
        budgeting_period_factory: FactoryMetaClass,
        ...
        """
        GIVEN: Multiple budgets with categories and transfers.
        WHEN: get_categories_transfers_sums_in_period called for specific budget.
        THEN: Only transfers from specified budget included.
        """
        budget1 = budget_factory(members=[base_user])
        budget2 = budget_factory(members=[base_user])
    ...
    def test_multiple_categories_same_period(
        ...
        category3 = transfer_category_factory(budget=budget, deposit=deposit, name="Cat 3")
