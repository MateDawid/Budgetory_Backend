...
class TestIncomeFilterSetFiltering:
    ...
    def test_get_transfers_list_filtered_by_empty_entity(
        ...
        income_factory(budget=other_budget, entity=entity_factory(budget=other_budget))
        income_factory(budget=other_budget, entity=None)
        income_factory(budget=budget, entity=entity_factory(budget=budget))
        matching_transfer = income_factory(budget=budget, entity=None)
    ...
    def test_get_transfers_list_filtered_by_empty_category(
        ...
        income_factory(
            budget=other_budget,
            category=transfer_category_factory(category_type=CategoryType.INCOME, budget=other_budget),
        )
        income_factory(budget=other_budget, category=None)
        income_factory(
            budget=budget, category=transfer_category_factory(category_type=CategoryType.INCOME, budget=budget)
        )
